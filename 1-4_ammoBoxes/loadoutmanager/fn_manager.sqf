...
_fnc = param [0,"init",[""]];
_params = param [1,[]];
_display = uiNamespace getVariable ["mar_managerGUI",displayNull];
_r = false;
...
switch (toLower _fnc) do {
	...
	case "init": {
		...
		_box setVariable ["mar_loadoutManager",true];
		
		// get favorites list & add them to box
		_favs = profileNamespace getVariable ["mar_Lo_favs",[]];
		{ 
			private "_blah"; 
			_blah = [_x,[_box]]; 
	...
	case "open": {
		...
		// params
		_box = _params param [0,objNull,[objNull]];
		missionNamespace setVariable ["mar_managerBOX",_box];
		
		// diag
		_diag = createDialog "mar_loadoutManager";
		disableSerialization;		
		_display = uiNamespace getVariable ["mar_managerGUI",displayNull];
		
		// hide ctrls
		{ CTRL(_x) ctrlShow false } forEach mar_SAVE_GROUP;
		{ CTRL(_x) ctrlShow false } forEach mar_PREV_GROUP;
		{ CTRL(_x) ctrlShow false; CTRL(_x) ctrlEnable false; } forEach mar_EXTRA_BUTTONS;
		
		// populate	
		GUI_REFRESH(-1)
		
		// change icon tooltip & call advanced hint on mouse click
		CTRL(mar_CTRL_HELPTIP) ctrlSetTooltip (mar_HELP_TIPS select floor random count mar_HELP_TIPS);
		CTRL(mar_CTRL_HELPTIP) ctrlSetEventHandler ["MouseButtonClick",
			format [
				'
					uiNamespace setVariable ["BIS_fnc_advHint_hintHandlers",true];
					[["mar_manager","mar_usage"], 30,"", 20,"", true, true, false, true] call BIS_fnc_advHint;
					((uiNamespace getVariable "mar_managerGUI") displayCtrl %1) ctrlSetTooltip (%2 select floor random count %2);
				',
				mar_CTRL_HELPTIP,
				mar_HELP_TIPS
			]
		...
		CTRL(mar_CTRL_SAVEDLIST) ctrlSetEventHandler ["keyDown",
			"if (_this select 1 in [211]) then {[""buttons"",[(uiNamespace getVariable 'mar_managerGUI') displayCtrl 15051]] call mar_fnc_manager;};"				
		];
	...
	case "listgear": {
		...
		_tree = _params param [0,CTRL(mar_CTRL_GEARTREE)];
		...
		_expand = ctrlIDC _tree == mar_CTRL_GEARTREE;
		_checking = mar_managerBOX getVariable ["mar_boxRestrictions",true];
		tvClear _tree;
	...
	case "listsaved": {
		...
		_index = _params param [0,lbCurSel CTRL(mar_CTRL_SAVEDLIST)];
		_savedList = CTRL(mar_CTRL_SAVEDLIST);
		// loop through vars, get names & add to array
		marLoadOuts = [];
		for "_num" from 1 to 20 do {
			call compile format ["
				mar_Lo_%1 = profileNamespace getVariable [""mar_Lo_%1"",[]];
				if ((count mar_Lo_%1) == 0) then {
					profileNamespace setVariable [""mar_Lo_%1"",[""Empty Loadout %1""]];
					mar_Lo_%1 = profileNamespace getVariable [""mar_Lo_%1"",[]]; 
				};
				marLoadOuts pushBack mar_LO_%1;
				",
				_num
			];		
		};
	...
	case "listdefault": {
		...
		_defaultList = CTRL(mar_CTRL_DEFAULTLIST);
		lbClear _defaultList;
		_defaultList lbSetCurSel -1;
		// get configured loadouts
		_cfg = configFile >> "CfgVehicles" >> typeOf mar_managerBOX;
		if (isArray (_cfg >> "mar_LOADOUTS")) then {
			private "_array";
			_array = getArray (_cfg >> "mar_LOADOUTS");
			...
			CTRL(mar_CTRL_BUTTONDEFAULT) ctrlEnable false;
		};
	...
	case "playergear": {
		...
		private["_mar_xtendMag","_mar_headgear","_mar_goggles","_mar_uniform","_mar_uniformitems","_mar_vest","_mar_vestitems","_mar_asgnItems","_mar_backpack","_mar_packitems","_mar_PrimaryGun","_mar_pgunitems","_mar_SecondaryGun","_mar_sgunitems","_mar_handgun","_mar_handgunitems","_mar_swFreqs","_mar_lrFreqs"];
		_r = [];
		
		_mar_headgear = headgear player;	
		_mar_goggles = Goggles player;
		_mar_uniform = uniform player;
		_mar_uniformitems = uniformItems player;
		_mar_vest = vest player;
		_mar_vestitems = vestItems player;
		_mar_asgnItems = assignedItems player;		
		_mar_backpack = backpack player;
		_mar_packitems = backpackItems player;		
		_mar_PrimaryGun = primaryWeapon player;
		_mar_pgunitems = primaryWeaponItems player;
		_mar_SecondaryGun = secondaryWeapon player;
		_mar_sgunitems = secondaryWeaponItems player;				
		_mar_handgun = handgunWeapon player;
		_mar_handgunitems = handgunItems player;
		
		// add loaded magazines as weapon items
		_mar_xtendMag = magazinesAmmoFull player;
		{
			switch (toLower (_x select 4)) do {
				case (toLower _mar_PrimaryGun): {_mar_pgunitems pushBack (_x select 0);};
				case (toLower _mar_SecondaryGun): {_mar_sgunitems pushBack (_x select 0);};
				case (toLower _mar_handgun): {_mar_handgunitems pushBack (_x select 0);};
				default {};
			};	
		} forEach _mar_xtendMag;
		...
		if ( { 
			...
			} forEach [_mar_uniformitems,_mar_vestitems,_mar_packitems];
		};
		
		// if has installed earbuds 
		if (mar_EARBUD_CHECK) then { _mar_uniformitems pushBack mar_CLASS_EARBUDS };
		...
		if !(isNil "_packWeps") then {	
			{	
				for "_i" from 1 to (count _x - 1) do {
					...
					if (_item != "") then { _mar_packitems pushBack _item };
				};
			} forEach _packWeps;
		};
		...
		if (TFAR_CHECK) then {
			if (call TFAR_fnc_haveSWRadio) then {
				...
				if !(isNil "_swSet") then {
					_mar_swFreqs = _swSet select 2;
				};
			};
			if (call TFAR_fnc_haveLRRadio) then {
				_lrSet = (call TFAR_fnc_activeLrRadio) call TFAR_fnc_getLrSettings;
				if !(isNil "_lrSet") then {
					_mar_lrFreqs = _lrSet select 2;
				};
			};
		};
		
		_r = [_mar_headgear,_mar_goggles,_mar_uniform,_mar_uniformitems,_mar_vest,_mar_vestitems,_mar_asgnItems,_mar_backpack,_mar_packitems,_mar_PrimaryGun,_mar_pgunitems,_mar_SecondaryGun,_mar_sgunitems,_mar_handgun,_mar_handgunitems,_mar_swFreqs,_mar_lrFreqs];
	...
	case "buttons": {
		...
		switch (ctrlIDC _button) do {
			...
			// main save button
			case mar_CTRL_BUTTONSAVE: {
				private "_heldCtrl";
				_heldCtrl = _params param [5,false];
				// show save contrls			
				if ( lbCurSel CTRL(mar_CTRL_SAVEDLIST) >= 0 ) then {				
					private ["_text","_empty"];
					
					// get text from list and set on edit box
					_index = lbCurSel CTRL(mar_CTRL_SAVEDLIST);
					_text = CTRL(mar_CTRL_SAVEDLIST) lbText _index;
					_empty = format ["Empty Loadout %1",_index + 1];
					if (_text == _empty) then {
						_text = STR_REPLACE(_text,"Empty ",""); 
					};
					CTRL(mar_CTRL_SAVEEDIT) ctrlSetText _text;
					
					// if ctrl is held, skip ctrlShow & call save action
					if _heldCtrl exitWith { 
						private "_quick";
						_quick = ["buttons",[CTRL(mar_CTRL_SAVESAVE)]] call FUNCTION_NAME; 
						_return = true;
					};
					
					// show & disable groups
					{ CTRL(_x) ctrlShow false } forEach mar_PREV_GROUP;
					{ CTRL(_x) ctrlShow true } forEach mar_SAVE_GROUP;
					CTRL(mar_CTRL_SAVERENAME) ctrlShow false; // disabled rename save
					{ CTRL(_x) ctrlEnable false } forEach mar_DISABLE_GROUP;
					CTRL(mar_CTRL_GEARTREE) ctrlSetTextColor [1,1,1,0.25]; // disabled color
					
					ctrlSetFocus CTRL(mar_CTRL_SAVESAVE);
			...
			// save cancel button
			case mar_CTRL_SAVECANCEL: { 
				// close save controls			
				{ CTRL(_x) ctrlShow false } forEach mar_SAVE_GROUP;
				{ CTRL(_x) ctrlEnable true } forEach mar_DISABLE_GROUP;
				CTRL(mar_CTRL_GEARTREE) ctrlSetTextColor (COLOR_WARNING_ARRAY); // tree text color
				_return = false;
			};	
			
			// save button/ pressing enter
			case mar_CTRL_SAVEEDIT; 
			case mar_CTRL_SAVESAVE: { 
				...
				ctrlSetFocus CTRL(mar_CTRL_HELPTIP); // stop close button getting hit
				
				// rename save button is shown
				if (ctrlShown CTRL(mar_CTRL_SAVERENAME)) exitWith {
					private "_rename";
					_rename =["buttons",[CTRL(mar_CTRL_SAVERENAME)]] call FUNCTION_NAME; 
					_return = true;
				};
				
				// name
				_index = lbCurSel CTRL(mar_CTRL_SAVEDLIST);
				_variable = format ["mar_Lo_%1",_index + 1];
				_empty = format ["Empty Loadout %1",_index + 1];
				
				// data
				_name = ctrlText CTRL(mar_CTRL_SAVEEDIT);	
			...
			case mar_CTRL_BUTTONDELETE: { 
				...
				if ( lbCurSel CTRL(mar_CTRL_SAVEDLIST) >= 0 ) then {
					...
					if (_found > -1) then {
						...
						profileNamespace setVariable ["mar_Lo_favs",_favs];
					};
			...
			case mar_CTRL_SAVEDLIST: {
				...
				if _heldShft exitWith { 
					private "_quick";
					_quick = ["buttons",[CTRL(mar_CTRL_FAVORITE)]] call FUNCTION_NAME; 
					_return = true;
				};
				...
				if ( lbCurSel CTRL(mar_CTRL_SAVEDLIST) >= 0 ) then {
					...
					if _heldCtrl exitWith {	
						private "_text";					
						_text = CTRL(mar_CTRL_SAVEDLIST) lbText (lbCurSel CTRL(mar_CTRL_SAVEDLIST));
						mar_renameIndex = lbCurSel CTRL(mar_CTRL_SAVEDLIST);
						// hide/show ctrls
						{ CTRL(_x) ctrlShow false } forEach mar_PREV_GROUP;
						{ CTRL(_x) ctrlShow true } forEach mar_SAVE_GROUP;
						CTRL(mar_CTRL_SAVESAVE) ctrlShow false; // hide gear save
						{ CTRL(_x) ctrlEnable false } forEach mar_DISABLE_GROUP;
						CTRL(mar_CTRL_GEARTREE) ctrlSetTextColor [1,1,1,0.25]; // disabled color
					...
					{ CTRL(_x) ctrlShow true } forEach mar_PREV_GROUP;
					_gear = ["listGear",[CTRL(mar_CTRL_PREVTREE),_array select 1]] call FUNCTION_NAME;
					ctrlSetFocus CTRL(mar_CTRL_PREVCLOSE);
					CTRL(mar_CTRL_PREVTEXT) ctrlSetText (_array select 0);
					{ CTRL(_x) ctrlEnable false; } forEach [mar_CTRL_GEARTREE,mar_CTRL_PREVBG];
			...
			// rename save button
			case mar_CTRL_SAVERENAME: {			
				private ["_index","_variable","_array","_name","_LO","_favs"];
				// data
				_index = lbCurSel CTRL(mar_CTRL_SAVEDLIST);
				if (_index == -1) then {_index = mar_renameIndex;};
				_variable = format ["mar_Lo_%1",_index + 1];
				_array = profileNamespace getVariable [_variable,[]];
				...
				if (_LO in _favs) then {
					{	// rename on all boxes
						// if has manager
						if ( _x getVariable ["mar_loadoutManager",false] ) then {
							private ["_index","_box_favs"];
							_box_favs = _x getVariable ["mar_loadoutManager_favs",[]];
							// get sub array index 
							_index = -1;
							{ 
			...
			// gear preview close
			case mar_CTRL_PREVCLOSE: {
				// hide preview group
				{ CTRL(_x) ctrlShow false } forEach mar_PREV_GROUP;
				CTRL(mar_CTRL_GEARTREE) ctrlEnable true;
				_return = true;
			};
			
			// set favorite
			case mar_CTRL_FAVORITE: {
				if ( lbCurSel CTRL(mar_CTRL_SAVEDLIST) >= 0 ) then {
	...
	case "loadsaved": {
		...
		_index = _params param [0,(lbCurSel CTRL(mar_CTRL_SAVEDLIST)),[0]];
		//_box = _params param [1,mar_managerBOX];
		_box = _params param [1,(missionNamespace getVariable ["mar_managerBOX",objNull])];
		missionNamespace setVariable ["mar_managerBOX",_box]; // define for favorites
		//mar_managerBOX = _box;
		
		// no selection
		if (_index < 0) exitWith {
			// check for default selection
			if (lbCurSel CTRL(mar_CTRL_DEFAULTLIST) > -1) then {
				private "_default";
		...
		// data
		_name = format ["mar_Lo_%1",_index + 1];
		...
		_checking = _box getVariable ["mar_boxRestrictions",true];
		_boxGear = CALL_FNC("boxGear",[]);
		...
		/*
		_mar_headgear = _gear select 0; //
		_mar_goggles = _gear select 1; //
		_mar_uniform = _gear select 2; //
		_mar_uniformitems = _gear select 3; //
		_mar_vest = _gear select 4; // 
		_mar_vestitems = _gear select 5; //
		_mar_asgnItems = _gear select 6; //
		_mar_backpack = _gear select 7; //
		_mar_packitems = _gear select 8; //
		_mar_PrimaryGun = _gear select 9; //
		_mar_pgunitems = _gear select 10; //
		_mar_SecondaryGun = _gear select 11; //
		_mar_sgunitems = _gear select 12; //
		_mar_handgun = _gear select 13; //
		_mar_handgunitems = _gear select 14; //
		_mar_swFreqs = _gear select 15; // <-- new
		_mar_lrFreqs = _gear select 16; // <-- new
		*/	
	...
	case "loaddefault": {
		...
		_null = _file spawn {
			private "_handle";
			_handle = [nil,player] execVM (mar_LOADOUT_ROOT + _this);
	...
	case "favorites": {
		...
		// run on all sent boxes
		{
			...
			if ( _x getVariable ["mar_loadoutManager",false] ) then {
				...
				// save new array
				_x setVariable ["mar_loadoutManager_favs",_box_favs];
